...
export const QuickQuote = ({ onViewChange }) => {
  ...
  return (
    <div className="page-content">
      <Header title="Quick Quote" subtitle="Create a simple quote in 60 seconds" showNewQuote={false} mode="demo">
        <button
      ...
      <div style={{ maxWidth: 880, margin: '0 auto' }}>
            ...
            {showCustomerDropdown && !selectedCustomer && filteredCustomers.length > 0 && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, marginTop: 4, boxShadow: '0 8px 24px rgba(0,0,0,0.1)', zIndex: 100, maxHeight: 220, overflowY: 'auto' }}>
                {filteredCustomers.map(c => (
                  ...
                  >
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: c.gradient || '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, color: '#fff', flexShrink: 0 }}>
                      {c.initials}
                    </div>
                    <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, paddingBottom: 40 }}>
          <button className="btn-secondary" onClick={() => triggerDemoPopup()}>Save</button>
          <button className="new-quote-btn" onClick={() => triggerDemoPopup()}>Save &amp; Convert to Booking</button>
        </div>
      </div>
    </div>
  );
