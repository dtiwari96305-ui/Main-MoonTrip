...
/**
 * Geo-coordinate lookup for Indian destinations.
...
/**
 * Try to find geo coordinates for a destination string.
 * Matches against substrings of the name.
 */
...
export const RealIndiaMapD3 = ({ destinations = [] }) => {
  ...
  useEffect(() => {
    ...
    fetch('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json')
      ...
      .then(world => {
        ...
        svg.append('defs')
          .append('clipPath').attr('id', 'realIndiaClip')
          .append('rect')
            .attr('x', 0).attr('y', cropTop)
            .attr('width', newW).attr('height', newH);

        const g = svg.append('g').attr('clip-path', 'url(#realIndiaClip)');
        ...
        // Dynamic markers from destinations prop
        const mk = g.append('g');
        const places = [];
        const seen = new Set();
        ...
        places.forEach(p => {
          ...
          mg.on('mouseenter', function () {
            ...
            if (prev && prev !== p.id) {
              const prevDot = d3.select(`#rimk-${prev} circle:nth-child(2)`);
              const prevRing = d3.select(`#rimk-${prev} circle:nth-child(1)`);
