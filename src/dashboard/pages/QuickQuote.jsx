...
export const RealQuickQuote = ({ onViewChange }) => {
  const { customers = [], vendors = [], addQuote, addVendor } = useData();
                ...
                {services[svc.key] && (
                  <div className="cq-simple-labeled-row" style={{ padding: '4px 4px 14px' }}>
                    ...
                    <div className="cq-simple-field">
                      ...
                      <VendorSelectDropdown value={serviceVendors[svc.key] || ''} onChange={v => setServiceVendors(prev => ({ ...prev, [svc.key]: v }))} vendors={vendors} onVendorAdded={addVendor} mode="real" className="cq-simple-vendor-in" placeholder="Search vendor..." />
                    </div>
                  </div>
                )}
              </div>
