...
const FALLBACK = [{ lat: -8.3405, lon: 115.0920 }];
...
export const WorldMapD3 = ({ destinations = [] }) => {
  ...
  const placesToRender = destinations.length > 0
    ? destinations.map((d, i) => {
        ...
        return geo ? { id: `wdest-${i}`, lat: geo.lat, lon: geo.lon } : null;
      }).filter(Boolean)
    : FALLBACK.map((p, i) => ({ id: `wfb-${i}`, ...p }));
