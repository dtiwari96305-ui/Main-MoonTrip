...
export const CustomersTable = ({ customers = [], searchQuery = '', onFilteredChange, onEdit, onDelete }) => {
  ...
  }), [customers, searchQuery, inlineSearchQuery]);
          ...
          {filtered.length > 0 ? filtered.map((c) => (
            <tr key={c.id} className="animate-row">
              ...
              <td className="dt-actions">
                <button className="action-btn edit-btn" title="Edit" onClick={() => onEdit && onEdit(c)}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                </button>
                <button className="action-btn delete-btn" title="Delete" onClick={() => onDelete && onDelete(c)}>
